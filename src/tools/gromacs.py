...
def read(gro_file, top_file="", doRegularExcl=True):
    ...
    nbparams={} # a dict: key (atomtypeid,atomtypeid) : class storing special nb parameters
    ...
    if top_file != "":
        ...
        readdefaults, readattypes, readnbparams, readbdtypes, readantypes, readdhtypes, readimptypes, read14pairs, determine_functiontype  = False, False, False, False, False, False, False, False, False
        defaults={} # gromacs default values
        ...
        for line in f.lines:
            ...
            if readnbparams:
                if line.strip() == "" or '[' in line: # end of nbparams section
                    readnbparams = False
                else:
                    tmp = line.split()
                    ...
                    a = (i,j)
                    print line
                    tmpprop = {"sig":float(tmp[3]),"eps":float(tmp[4])}
                    if a not in nbparams:
                        nbparams.update({a:tmpprop})
    ...
    if len(nbparams) !=0:
        print "Found ", len(nbparams), " special nb parameters"
        unpackvars.append("nbparameters")
        params.append(nbparams)
    if len(bonds) != 0:
...
def setLennardJonesInteractions(system, defaults, atomtypeparams, verletlist, cutoff, hadress=False, adress=False, ftpl=None, specialNB={}):
    ...
    if defaults:
        if int(defaults['combinationrule'])==1:
            ...
            for pair in specialNB.iteritems():
                c6=float(pair['sig'])
                c12=float(pair['eps'])
                if c6==0: continue
                sig = pow(c12/c6,1.0/(6.0))
                ...
                pair['sig']=sig
                pair['eps']=eps
                print "WARNING: Converted special nb pair ", pair, "to sigma, epsilon parameters", " sig= ", sig, " eps=", eps
    ...
    for i in atomtypeparams.keys():
         for j in atomtypeparams.keys():
            ...
            if i>j:
                pair=(j,i)
            if pair in specialNB:
                sig=float(specialNB[pair]['sig'])
                eps=float(specialNB[pair]['eps'])
            elif pi!=pj:
